...
[[ -f $root/manifest.json ]] || die "Not an omalookout checkout: missing $root/manifest.json"
...
unlink() {
  ...
  if [[ -d $backup ]]; then
    ...
    printf 'Unlinked %s. No clone to restore; omarchy plugin add https://github.com/shieldsworks/omalookout.git --enable\n' "$target"
  fi
  rescan
}
